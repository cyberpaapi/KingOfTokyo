...
export default function Home() {
  ...
  const play = useCallback((sound: "dice-roll" | "smash-impact" | "energy-zap" | "heal-pulse" | "victory-roar") => { if (!soundOn) return; const audio = new Audio(`/sounds/${sound}.mp3`); audio.volume = .55; void audio.play().catch(() => {}); }, [soundOn]);
  const sync = useCallback(async (next: GameState) => { if (!room) return; const response = await fetch(`/api/rooms/${room.code}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ token: room.token, action: "sync", state: next, revision: revisionRef.current }) }); if (response.ok) { const data = await response.json(); revisionRef.current = data.revision; setRoom((r) => r ? { ...r, revision: data.revision } : r); } }, [room]);
